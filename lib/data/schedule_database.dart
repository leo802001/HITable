...
class ScheduleDatabase {
  ...
  /// 读取界面外观设置。缺失的键由 [AppearanceSettings.fromKeyValues] 回退默认值，
  /// 因此老用户升级后不需要数据迁移。
  Future<AppearanceSettings> loadAppearanceSettings() async {
    final values = await _readSettings(prefix: 'appearance.');
    return AppearanceSettings.fromKeyValues(values);
  }

  /// 保存界面外观设置。
  Future<void> saveAppearanceSettings(AppearanceSettings settings) async {
    await _writeSettings(settings.toKeyValues());
  }

  /// 按前缀读取 app_settings，返回 `key -> value`。
  Future<Map<String, String>> _readSettings({String? prefix}) async {
    final rows = await database.query(
      'app_settings',
      where: prefix == null ? null : 'key LIKE ?',
      whereArgs: prefix == null ? null : ['$prefix%'],
    );
    return {
  ...
  /// 批量写入 app_settings（单事务，失败不留半截状态）。
  Future<void> _writeSettings(Map<String, String> values) async {
    await database.transaction((txn) async {
