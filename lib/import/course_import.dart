...
import 'hit_timetable_import.dart';
...
class CourseImportParser {
  ...
  /// 解析老版 `.xls`（OLE2 / BIFF8）。
  ///
  /// pub 的 `excel` 包**不支持 .xls**，因此这里自己读网格。
  ///
  /// 教务导出的 .xls 是**网格式**课表（星期为列、节次为行），
  /// 与 [HitTimetableImportParser] 期望的结构一致 —— 因此先由它解析，
  /// 再用 [HitTimetableImportParser.toImportPreview] 转成通用表格，
  /// 复用「课程名\n教师[周次]周地点」这一整套已适配好的逻辑。
  ImportPreview parseXls(Uint8List bytes) {
    ...
    if (workbook.isEmpty) {
      return const ImportPreview(
        ...
        errors: [
          '无法从该 .xls 中读出课表数据。\n'
              '请确认它是教务系统导出的课表文件（老版 Excel 格式）。',
        ],
      );
    }
    final result = const HitTimetableImportParser().parse(workbook.grid);
    if (result.records.isEmpty) {
      return ImportPreview(
        ...
        errors: [
          '读到了表格，但没有识别出课程。\n'
              '请确认这是教务导出的课表（含「第N,M节」行与星期表头）。',
          ...result.warnings,
        ],
      );
    }
    return const HitTimetableImportParser().toImportPreview(result);
  }
