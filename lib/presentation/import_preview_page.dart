  ...
  Future<void> _pickFile() async {
    ...
    try {
      ...
      final preview = switch (extension) {
        'csv' => _parser.parseCsv(utf8.decode(bytes, allowMalformed: true)),
        // 老版 .xls（OLE2）与新式 .xlsx 是两种完全不同的格式，
        // excel 包只认后者，因此 .xls 走自研读取器
        'xls' => _parser.parseXls(bytes),
        _ => _parser.parseXlsx(bytes),
      };
