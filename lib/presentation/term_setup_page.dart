...
class _TermSetupPageState extends ConsumerState<TermSetupPage> {
  ...
  void initState() {
    ...
    // 新建学期时默认用学校公布的开学第一周周一（2026-08-31），
    // 避免不清楚该填哪天的同学填错。用户仍可改成任意日期。
    _firstMonday = term?.firstWeekMonday ?? defaultFirstWeekMonday;
    _periods = List.of(term?.periodsByWeekday[1] ?? hitLessonPeriods);
  }
  ...
  Future<void> _importTermSettings() async {
    ...
    try {
      ...
      if (extension == 'csv') {
        ...
        // 老版 .xls 与 .xlsx 格式不同，excel 包只认后者
        suggestion = _termParser.parseXls(await file.readAsBytes());
      } else {
